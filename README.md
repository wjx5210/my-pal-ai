...
| 可观测性 | 请求 ID、访问耗时日志、检索调试输出、可选 Prompt/回答日志 |
| 会话能力 | 最近 12 条消息记忆、追问检索、本地历史恢复 |
| 测试规模 | 44 项，包含单元、API、集成和在线 AI 评估测试 |
...
- **可测试、可观测**：44 项测试覆盖数据、检索、AI 问答、API 和会话流程；每个 HTTP 请求返回 `X-Request-ID`，后端记录路径、状态码与耗时，便于关联线上现象和容器日志。
...
运行默认离线回归测试（不会调用真实模型 API）：

```bash
python -m pytest -q
```

运行在线 AI 评估测试（会产生外部 API 调用和费用）：

```bash
python -m pytest -m ai -q
```

运行全部测试：

```bash
python -m pytest -m "ai or not ai" -q
```

当前共收集 44 项测试，其中默认离线回归 41 项、在线 AI 评估 3 项，覆盖：
...
带有 `ai` 标记的测试会调用真实的 DeepSeek 或百炼 API，并要求已经生成 `data/vector_store.json`。默认执行会排除这些测试，作为每次改动必须通过的快速质量门禁；在线评估需要显式执行，并在运行前配置两个 API Key、构建向量库和确认网络可用。
